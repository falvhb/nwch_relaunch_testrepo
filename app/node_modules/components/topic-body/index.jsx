...
const TopicBody = React.createClass({
  ...
  getFromApi(topic, page) {
    ...
    axios.get(this.getApiUrl(topic, page))
      .then(function (response) {
        var obj = {
          articles: _.get(response, 'data.data')
        };
        self.setState({
  ...
  mapTeaserItems(results) {
    return results.map((article, index) => {
      return <Teaser key={index} {...article} modifier={index < 6 ? 'basic' : 'basic-listed'}/>;
    });
  },
    ...
    return (
      <div {...classes()}>
        {results.length > 0
          ...
          ? <Grid modifier='three-up-simple' items={this.mapTeaserItems(results)} />
        ...
        {page < totalPages
          ...
          ? infiniteLoad
              ...
              ? <div {...classes('load-more', null, loading ? 'is-loading' : null)}>
                  <Button _event={this.loadMore} label='Mehr Artikel' />
                </div>
