import React from 'react';
import bem from 'react-bem-helper';
import KalturaAPI from 'helpers/kaltura-api';

import Container from 'components/container';
import VideoLibraryHeader from 'components/video-library-header';
import VideoLibraryList from 'components/video-library-list';
import { getScript } from 'helpers';

var libraryIncluded = false;

const classes = new bem({
  name: 'video-library',
  prefix: ''
});

const VideoLibrary = React.createClass({

  displayName: 'VideoLibrary',

  statics: {
    loadKalturaClient: function() {
      if (libraryIncluded) {
        return;
      }

      // @TODO: maybe use promise based script loader later on
      getScript('/client/scripts/vendor/KalturaClient.min.js', () => {
        libraryIncluded = true;
      });
    },
    // loading statuses
    IS_LOADED: 0,
    IS_LOADING: 1,
    LOADING_FAILED: 2
  },

  propTypes: {
    initialVideos: React.PropTypes.arrayOf(React.PropTypes.object),
    categoryId: React.PropTypes.string.isRequired,
    pageSize: React.PropTypes.number.isRequired
  },

  getInitialState() {
    var initialVideos = this.props.initialVideos || [];
    return {
      selectedVideo: null,
      videos: initialVideos,
      loadingStatus: VideoLibrary.IS_LOADED,
      searchText: '',
      pageIndex: 1,
      pageSize: this.props.pageSize,
      categoriesIdsMatchAnd: ''
    };
  },

  changeSearchText() {
    this.setState({
      searchText: event.target.value
    });
  },

  selectVideo(videoID) {
    this.setState({selectedVideo: videoID});
  },

  deselectVideo() {
    this.setState({selectedVideo: null});
  },

  toggleVideo(videoID) {
    if (this.state.selectedVideo === videoID) {
      this.deselectVideo();
    } else {
      this.selectVideo(videoID);
    }
  },

  // removes unwanted elements at the end of the array and adds new items to array at the end
  addVideoPage(videos) {
    var updatedVideos = this.state.videos;
    var start = (this.state.pageIndex-1) * this.props.pageSize;
    updatedVideos = updatedVideos.slice(0, start);
    updatedVideos = updatedVideos.concat(videos);

    return updatedVideos;
  },

  setVideos(videos) {
    var updatedVideos = this.addVideoPage(videos);
    this.setState({
      videos: updatedVideos
    });
  },

  search() {
    // reset paging when doing keyword search
    this.setState({
      pageIndex: 1
    });

    this.loadVideos();
  },

  increasePageIndex() {
    var nextIndex = this.state.pageIndex + 1;
    this.setState({
      pageIndex: nextIndex
    });
    // console.log('PROBLEM: state.pageIndex ist not set when loadVideos() is called: ', 'nextIndex=', nextIndex, 'pageIndex=', this.state.pageIndex);
    //
    // Problem: state.pageIndex not holding new value when loadVideos using its value
    // this.loadVideos();
  },

  loadVideos() {
    this.setState({
      loadingStatus: VideoLibrary.IS_LOADING
    });

    var kalturaAPI = new KalturaAPI({
      ssl: false,
      domain: 'www.aargauerzeitung.ch',
      path: '/__kaltura_api_proxy__',
      service: 'baseEntry',
      action: 'list',
      format: 1,
      orderByKey: 'createAt',
      orderDescending: true,
      freeText: this.state.searchText,
      mediaTypeEqual: 1,
      categoriesIdsMatchAnd: this.state.categoriesIdsMatchAnd,
      pageIndex: this.state.pageIndex,
      pageSize: this.state.pageSize
    });

    // load videos using promises
    kalturaAPI.getVideos().then((response) => {
      this.setVideos(response.data);
      this.setState({
        loadingStatus: VideoLibrary.IS_LOADED
      });
    }).catch(function () {
      // console.log('error loading videos', this.state.loadingStatus);
      this.setState({
        loadingStatus: VideoLibrary.LOADING_FAILED
      });
    });
  },

  componentDidMount() {
    VideoLibrary.loadKalturaClient();
  },

  shouldComponentUpdate() {
    // console.log('shouldComponentUpdate');
    return true;
  },

  componentDidUpdate(prevProps, prevState) {
    if (prevState.pageIndex !== this.state.pageIndex) {
      // console.log('state searchText about to change: ', prevState.pageIndex, this.state.pageIndex);
      //
      // @TODO: @Richard:
      // This feels like a hack to have the loading of videos in here separated from setting the state.
      // QUestion: What are the best practices for invoking the loadVideo() on increase of the page index?
      //
      // When I do this
      //
      // this.setState({pageIndex: pageIndex + 1})
      // this.loadVideos();
      //
      // in increasePageIndex() state.pageIndex used inside loadVideos() still has the old value (!)
      // There seems to be no other way of doing it as to use componentWillUpdate lifecycle method to trigger loading.. ?
      //
      this.loadVideos();
    }
  },

  render() {
    return (
      <div {...classes()}>
        <Container>
          <VideoLibraryHeader
            changeSearchText={this.changeSearchText}
            onSearch={this.search}
          />
          <VideoLibraryList
            videos={this.state.videos}
            selectedVideo={this.state.selectedVideo}
            playerID={this.props.playerID}
            accountID={this.props.accountID}
            onToggleVideo={this.toggleVideo}
            onNextPage={this.increasePageIndex}
            loadingStatus={this.state.loadingStatus}
          />
        </Container>
      </div>
    );
  }

});

export default VideoLibrary;
