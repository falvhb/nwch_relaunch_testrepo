...
export default React.createClass({
    ...
    return items.map((item, index) => {
      ...
      if (selectedVideo && selectedVideo === this.props.selectedVideo) {
        var caption = {
          ...
          source: "@TODO: Source needed from extended meta data or from source:-tag of Kaltura"
        };
        var meta = {
  ...
  render() {
    ...
    if (this.props.loadingStatus === VideoLibrary.LOADING_FAILED) {
      errorMessage = 'Leider konnten die Videos nicht geladen werden. Bitte versuchen Sie es erneut.';
    }
    if (!videoTeasers.length) {
      errorMessage = 'Leider konnten keine Videos zu Ihrem Suchbegriff gefunden werden.';
    }

    // show more button if no error occured during loading
    if (!errorMessage) {
      content = this.mapTeaserItems(videoTeasers);
      loadMore = (
        <div {...classes('load-more', null)} key="showmoreContainer">
          <Button
            label="Mehr"
            _target="#"
      ...
      // display showmore button only if the total count for videos has not been reached yet
      if (this.props.totalCount > this.props.videos.length) {
