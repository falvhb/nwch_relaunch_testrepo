...
const ImageGallery = React.createClass({
  ...
  nextSlide() {
    this.flickity.next();
    this.updateHistory(this.flickity.selectedIndex)
  },
  ...
  updateHistory(index) {
    history.replaceState(null, document.title, '?slider'+this.id+'='+index);
  },

  render() {
    return (
      <div ref='imageGalleryElement' {...classes(null, null, [this.state.loading ? 'is-gallery-loading' : null])}>
        <nav {...classes('nav')}>
          ...
          <div {...classes('nav-bar')}>
            <span {...classes('slide-number')}>{this.state.currentIndex + 1} / {this.props.images.length}</span>
        ...
        {this.props.images.map((image, index) =>
          <div {...classes('item')} key={index}>
            <ImageAsset asset={image} size='max' letterbox={true} ratio='16x9' defer={true} />
          </div>
        )}
        </div>
      </div>
