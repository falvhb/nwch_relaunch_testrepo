...
export default Component => React.createClass({
  ...
  propTypes: {
    ...
    stoerer: React.PropTypes.arrayOf(React.PropTypes.object).isRequired,
    keywords: React.PropTypes.arrayOf(React.PropTypes.string).isRequired
  },

  mapMainAsset() {
    if (this.props.assets.length) {
      return <GetEnrichment enrichment={this.props.assets[0]} />;
    }
  },
  ...
  mapContentAndEnrichments() {
    ...
    while (index > -1) {
      ...
      var enrichment = this.props.stoerer[enrichmentIndex];
      if (enrichment) {
  ...
  mappedData() {
    return {
      asset: this.mapMainAsset(),
      intro: this.props.lead,
      body: this.mapContentAndEnrichments(),
