...
.teaser--default {
  overflow: hidden; /* So Text doesn’t overflow on Mobile */
}
...
/**
 * Teaser footer
 * 1. Don't wrap text around image when floated
 */
 ...
 * 1. Give some room between the image and the text
 * 2. Prevent the text from wrapping under the image
 */
...
.teaser--large .teaser__image::before {
  @include media('>500px') {
    ...
    left: 0; /* 1 */
    z-index: 3; /* 2 */
    background-image: linear-gradient(to bottom, rgba(255, 255, 255, 0), rgba(0, 0, 0, 0.5) 70%, #000 100%);
  }
}
...
@include media('<=500px') {
   ...
   * 1. Don't wrap text around image when floated
   */
  .teaser--basic-listed .teaser__footer {
    overflow: hidden; /* 1 */
  }

  /**
   * Image for the left inlined versiond
   */
  ...
  /**
   * Modify the container into a square
   */
  .teaser--basic-listed .teaser__image .responsive-image::before {
    padding-bottom: 100%;
  }

  /**
   * Scale the image to fill the square
   * Always scaling from 16:9 to 1:1, so percentages are constant
   */
  .teaser--basic-listed .teaser__image .responsive-image__img {
    width: 177.778%;
    margin-left: -38.8889%;
  }
   ...
   * Make the primary and tertiary content on the same line
   */
  .teaser--basic-listed .teaser__title {
    display: inline;
  }
   ...
   * 1. Make the primary and tertiary content on the same line
   */
  .teaser--basic-listed .teaser__supplementary {
    display: inline; /* 1 */
    margin-left: 10px;
  }
}
