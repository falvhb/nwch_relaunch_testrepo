@import 'globals';

.teaser--large {
  @include font-smoothing;

  /**
   * Hack the image ratio, providing more balanced layout at medium screens
   */
  @include media('>grid-break-2', '<=900px') {
    @include fit-image-to-box((16, 9), (4, 3));
  }
}


/**
 * Teaser link
 * 1. Position context for footer
 * 2. Link hover style
 */
.teaser--large .teaser__link {
  @include media('>grid-break-1') {
    position: relative; /* 1 */
  }

  @include on-interaction { /* 2 */
    .teaser-title__text-wrap {
      @include media('>grid-break-1') {
        box-shadow: inset 0 -1px 0 white;
      }
    }
  }
}


/**
 * Remove the bottom margin of the image for the large variation
 */
.teaser--large .teaser-image {
  @include media('>grid-break-1') {
    margin: 0;
    opacity: 0.9;
  }
}


/**
 * Add a top transparent black layer to be able to read text on top of image
 * 1. Stretching
 * 2. On top of image
 */
.teaser--large .teaser-image::before {
  @include media('>grid-break-1') {
    content: '';
    position: absolute; /* 1 */
    top: 0; /* 1 */
    right: 0; /* 1 */
    bottom: 0; /* 1 */
    left: 0; /* 1 */
    z-index: 3; /* 2 */
  }

  @include media('>grid-break-1', '<=grid-break-2') {
    @include image-gradient(25%);
  }

  @include media('>grid-break-2') {
    @include image-gradient(45%);
  }
}


/**
 * Teaser footer
 * 1. Overlapping image, from the bottom
 * 2. On top of image + transparent layer
 */
.teaser--large .teaser__footer {
  @include media('<=grid-break-1') {
    background-color: $light-grey;
  }

  @include media('>grid-break-1') {
    position: absolute; /* 1 */
    bottom: 0; /* 1 */
    left: 0; /* 1 */
    right: 0; /* 1 */
    z-index: 4; /* 2 */
    color: white;
  }

  @include media('<=950px') {
    padding: 16px;
  }

  @include media('>950px') {
    padding: 16px 20px;
  }
}


/**
 * Revisit the font styles for the secondary content
 */
.teaser--large .teaser__catchword {
  font-size: 13px;

  @include media('>grid-break-1') {
    color: shade(#fff, 20%);
  }
}
