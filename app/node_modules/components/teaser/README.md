...
Teasers nearly always called via `<tal:>` includes in the CMS. For this, a raw article is passed fron the API to [`slot.js`](/app/node_modules/components/teaser/slot.js). Here data is mapped and passed into the `Teaser` components.

As the full API response is so large, the [`getTeaserFromArticle`](/app/node_modules/helpers/get-teaser-from-article.jsx) helper maps only the data the `Teaser` component actually needs based on the variation / modifier.
