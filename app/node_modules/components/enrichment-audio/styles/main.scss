...
/**
 * Audio störer when inside an `.aside` (floating)
 * 1. Aside takes care of vertical rhythm already
 */
.aside .enrichment-audio {
  margin: 0; /* 1 */
...
/**
 * Footer for the audio, containing meta data
 * 1. Bottom border, ideally sync with the one from `.enrichment-text`
 * 2. Prevent text from sticking to audio and border
 * 3. Prevent text from sticking to edge of container
 */
.enrichment-audio__footer {
  float: left;
...
/**
 * Footer of full width audio störer on large screens
 */
.aside .enrichment-audio__footer {
  @include media('>small') {
    margin: 0;
  }
}
