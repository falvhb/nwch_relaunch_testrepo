...
export default React.createClass({
  ...
  getPlayerConfiguration(settings, includeAdvertising = true) {

    var fallbackSettings = {
      kalturaID: this.props.kalturaID,
      accountID: this.props.accountID,
      playerID: this.props.playerID
    };

    settings = _.isObject(settings) ? settings : _.extend({}, fallbackSettings);
  ...
  getPlayerElement() {
    return document.getElementById(this.props.kalturaID);
  },
  ...
  trackVideo() {
    console.log('trackity video');
    this.trackNetMetrix();
  },

  trackNetMetrix() {
    // create tracking pixel as image tag
    var img = document.createElement('img');
    img.src = this.props.trackingURL;
  },

  unmuteAndPlayPlayerWithAds() {
    var playerElement = this.getPlayerElement();
    // Hack to ensure that player is unmuted
    playerElement.sendNotification('changeVolume', 0.8);

    // auto play the video if autoPlay feature is deactivated
    // or if we are in non mobile browsers
    if (this.props.autoPlay && !isMobile()) {
      playerElement.sendNotification('doPlay');
    }

    // track NetMetrix on doPlay if autoPlay is deactivated
    // or we are in a mobile browser
    if (!this.props.autoPlay || isMobile()) {
      playerElement.kBind('doPlay.firstPlayTracking', () => {
        this.trackVideo();
        playerElement.kUnbind('doPlay.firstPlayTracking');
      });
    }
    ...
    return (
       <figure {...classes()} itemProp='video' itemScope itemType='http://schema.org/VideoObject'>
        <div {...classes('container')}>
          <div {...classes('player')} ref='videoPlayer' id={kalturaID} />
        </div>
        <Caption {...caption} />
