...
export default React.createClass({
  ...
  filterHtmlNodes(html) {
    ...
    if (videoRegex.test(filteredHtml)) {
      ...
      let wrapper = '<div class="enrichment-snippet__video">$&</div>';

      if (widthResults && heightResults) {
        let videoRatio = ((parseInt(heightResults[1]) / parseInt(widthResults[1])) * 100) + '%';
        wrapper = '<div class="enrichment-snippet__video" style="padding-bottom: ' + videoRatio + '">$&</div>';
      }

      filteredHtml = filteredHtml.replace(videoRegex, wrapper);
    }

    // Wrap trailing text (a caption) in a span
    const captionRegex = /((?![<>]).)*$/;
    filteredHtml = filteredHtml.replace(captionRegex, '<span class="enrichment-snippet__text">$&</span>');
