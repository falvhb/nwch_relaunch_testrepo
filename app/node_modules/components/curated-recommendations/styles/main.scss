...
.curated-recommendations__wrapper {
  margin-top: $container-spacing; /* 1 */
...
.curated-recommendations__wrapper--multiple {
  ...
  @include media('>small') {
    ...
    border-spacing: $container-spacing; /* 4 */
    margin-top: 0; /* 5 */
    width: auto; /* 5 */
  }

  @include media('>836px') { /* ($container-width + 2*$container-spacing) to compensate neg. margin */
    margin-left: -($container-spacing); /* 5 */
    margin-right: -($container-spacing); /* 5 */
  }
}
