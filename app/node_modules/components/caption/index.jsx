...
export default React.createClass({
  ...
  render() {
    ...
    if (source) cleanSource = capitalizeAll(source.replace('/', ' / '));

    const captionContents = (
      <div>
        {title && title !== description && title !== source
          ? <p {...classes('title', [(title && description) ? 'bold' : 'normal'])} itemProp='name'>{title}</p>
        :null}
        {description
          ? <p {...classes('description')} itemProp='description' dangerouslySetInnerHTML={{__html: description }} />
        :null}
        {source
          ? <p {...classes('source')} itemProp='author'>&copy; {cleanSource}</p>
        :null}
      </div>
    );
