...
export default React.createClass({
  ...
  renderScrollNavigation() {
    const { skin, path } = this.props;
    const navItems = this.navItems.slice();

    var segments = path.match(/\/(\w|-)+/g);
    var firstSegment = segments.length > 2 ? segments[0] : '/';
            ...
            <ul role='navigation'>
              ...
              {navItems.map((navItem, index) =>
                <li key={index}>
                  <a
                    href={navItem.href}
                    className={'header-nav__link' + (firstSegment === navItem.href ? ' header-nav__link--active' : '')}>
                    {navItem.label}
                  </a>
                </li>
              )}
        ...
        <div {...classes('brand')}>
          <div {...classes('container')}>
            ...
            <a href="/" title="Startseite">
              <img {...classes('logo', 'notext')} src={this.getLogoSource('logoMobile')} />
              <img {...classes('logo', 'text')} src={this.getLogoSource('logo')} />
            </a>
          </div>
        </div>
        <nav {...classes('container', [this.state.isHidden ? 'hidden' : null])}>
          <ul role='navigation'>
            <li>
              <a {...classes('link', 'login')} href="/login">
                <Icon id="user"/> Login
              </a>
            </li>
            {navItems.map((navItem, index) =>
              <li key={index}>
                <a {...classes('link', [firstSegment === navItem.href ? 'active' : null])} href={navItem.href}>
                  {navItem.label}
                </a>
              </li>
            )}
