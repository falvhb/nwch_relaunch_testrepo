...
.header-nav.is-slim .header-nav__brand {
  padding-top: 0.5em;
...
.header-nav__logo--slim .header-logo {
  width: 200px;
  height: 50px;
}
 ...
 * 1. Behind navigation
 * 2. On smaller screens, use max-height animation (don't fill viewport)
 * 3. On larger screens, use translate
 */
.header-nav__toggle {
  background-color: $blue;
  position: relative;
