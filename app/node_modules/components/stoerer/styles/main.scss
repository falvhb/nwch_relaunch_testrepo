...
.stoerer {
  ...
  padding: ($stoerer-basic-gap + 4px) $stoerer-basic-gap $stoerer-basic-gap; /* 1 */
  margin: ($stoerer-basic-gap * 2) -$stoerer-basic-gap; /* 2 */
  position: relative; /* 3 */
...
.stoerer--floating {
  @include media('>small') {
    float: left;
    padding: $stoerer-medium-gap;
    margin: 6px ($stoerer-medium-gap * 1.5) ($stoerer-medium-gap * 1.5) 0; /* 1 */
    width: $stoerer-width; /* 2 */

    /**
     * Adapt to updated padding
     */
    &::before {
      left: $stoerer-medium-gap;
      right: $stoerer-medium-gap;
    }
  }
