...
.enrichment--aside {
  ...
  @include media('>mobile-portrait') {
    float: left;
    margin: 6px $enrichment-floated-spacing $enrichment-floated-spacing 0; /* 1 */
  }

  @include media('>mobile-portrait', '<=700px') {
    // allow enrichments and ads next to each other for a bit
    width: $enrichment-width - 30px;
  }
  ...
  @include media('>full-screen-space') {
