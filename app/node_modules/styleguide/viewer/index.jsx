import React from 'react';
import ReactToJsx from 'react-to-jsx';
// import Highlight from 'react-highlight';
import bem from 'react-bem-helper';

import Tabs from 'styleguide/tabs';
import IFrame from 'styleguide/iframe';
import Loading from 'styleguide/loading';
import Container from 'styleguide/container';

const classes = new bem({
  name: 'viewer',
  prefix: 'styleguide-'
});

export default React.createClass({

  displayName: 'Viewer',

  propTypes: {
    component: React.PropTypes.object.isRequired,
    variation: React.PropTypes.object,
    children: React.PropTypes.element.isRequired
  },

  removeTabs(markup) {
    return markup.replace(/\t/g, '  ');
  },

  render() {
    var props = this.props;

    var sections = [
      {
        title: 'Preview',
        content: <IFrame {...props} />
      },
      {
        title: 'JSX',
        content: <Loading key='jsx'>{this.removeTabs(ReactToJsx(props.children))}</Loading>
        // content: <Loading key='jsx'><Highlight className='html'>{this.removeTabs(ReactToJsx(props.children))}</Highlight></Loading>
      },
      {
        title: 'HTML',
        content: <Loading key='jsx'>{this.removeTabs(ReactToJsx(props.children))}</Loading>
        // content: <Loading key='jsx'><Highlight className='html'>{this.removeTabs(ReactToJsx(props.children))}</Highlight></Loading>
      },
      {
        title: 'Data',
        content: props.json
          ? <Loading key='data'>{JSON.stringify(props.json, null, 2)}</Loading>
          // ? <Loading key='data'><Highlight className='json'>{JSON.stringify(props.json, null, 2)}</Highlight></Loading>
          : <p>You should add sample data to <code>{props.variation ? '.variations.json' : '.data.json'}</code> in the root of this component!</p>
      }
    ];

    return (
      <Container>
        <div {...classes()}>
          <Tabs sections={sections} />
        </div>
      </Container>
    );
  }

});
