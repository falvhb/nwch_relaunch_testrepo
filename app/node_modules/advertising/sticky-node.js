...
import getViewportWidth from 'helpers/get-viewport-width';

let stickyParams = {}, scrollEvent, stickyNodesCount = 0;
...
let toggleStickyScrolling = function() {
  ...
  if (getViewportWidth() > breakpoint) {
    ...
    let elHeightDifference = stickyParams.element.offsetHeight - (stickyParams.stopper ? stickyParams.stopper.offsetHeight : 0);
...
let scrollBehaviour = function(current) {
  let stickPosition = false;
...
let createStickyWrapper = function(el, i) {
  let org_html = el.innerHTML;
  let new_html = '<div id="stickyNode--' + i + '">' + org_html + '</div>';
