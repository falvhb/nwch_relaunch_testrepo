...
describe('Net Metrix Tracker\'s method', () => {
  ...
  describe('getTag', () => {
    it('returns a a valid net metrix tag', () => {
      ...
      delete data.referer;
      delete data.path.sitename;
      delete data.path.product;
      delete data.path.path;
      // delete data.path.view;
      // delete data.path.event;

      output = NetMetrixTracker.getTag(data);
      expect(output).to.contain('https://aznetz.wemfbox.ch/cgi-bin/ivw/CP/page/pageview');
  ...
  describe('isLiveHost', () => {
    it('uses global without_wemf flag', () => {
      window.az = {
        globals: {
          without_wemf: true
        }
      };
      output = NetMetrixTracker.isLiveHost();
      expect(output).to.be.false;

      window.az.globals.without_wemf = false;
      output = NetMetrixTracker.isLiveHost();
      expect(output).to.be.true;
    });

    it('returns true even if invalid parameter is given', () => {
      output = NetMetrixTracker.isLiveHost();
      expect(output).to.be.true;
    });
  });
  ...
  describe('trackPageView', () => {
    ...
    it('fires tracking', () => {
      let _track = NetMetrixTracker._track;
      sinon.spy(NetMetrixTracker, '_track');
      NetMetrixTracker.trackPageView(data);
      NetMetrixTracker._track.should.have.been.calledWithExactly(data, NetMetrixTracker.TYPE_PAGEVIEW);
      NetMetrixTracker._track.should.not.have.been.calledWithExactly(data, NetMetrixTracker.TYPE_EVENT);

      // restore method converted to spy
      NetMetrixTracker._track = _track;
    });

    it('fires tracking even if invalid parameter is given', () => {
      let _track = NetMetrixTracker._track;
      sinon.spy(NetMetrixTracker, '_track');
      NetMetrixTracker.trackPageView();
      NetMetrixTracker._track.should.have.been.calledWithExactly(undefined, NetMetrixTracker.TYPE_PAGEVIEW);
  ...
  describe('_requestTag', () => {
    it('requests an image by setting its src attribute', () => {
      let image = NetMetrixTracker._requestTag('http://www.dummy.com/tag');
      expect(image.src).to.equal('http://www.dummy.com/tag');
      expect(image instanceof Image).to.be.true;
      // make sure to remove image again
      image.parentNode.removeChild(image);
    });
  });

  describe('_track', () => {
    it('does fire tracking or just log if we are not on a live host', () => {
      let data = {
        domain: 'aznetz'
      };
      let t = NetMetrixTracker;

      let origGetTag = t.getTag;
      let origIsLiveHost = t.isLiveHost;
      let orig_requestTag = t._requestTag;
      let orig_log = t._log;

      let tag = t.getTag(data);

      let getTag = sinon.spy(NetMetrixTracker, 'getTag');
      let isLiveHost = sinon.spy(NetMetrixTracker, 'isLiveHost');
      let _requestTag = sinon.spy(NetMetrixTracker, '_requestTag');
      // make sure loging to console is not done when testing
      t._log = sinon.spy(function(tag, type) {});

      t._track(data, t.TYPE_PAGEVIEW);

      t.getTag.should.have.been.calledWithExactly(data);
      t.isLiveHost.should.have.been.called;
      t.isLiveHost.should.have.returned(true);
      t._requestTag.should.have.been.called;
      // t._requestTag.should.have.been.calledWithExactly(tag);
      t._log.should.not.have.been.called;
      ...
      t._track(data, t.TYPE_PAGEVIEW);
      t._log.should.have.been.called;
      // t._log.should.have.been.calledWithExactly(data, t.TYPE_PAGEVIEW);
      t._requestTag.should.not.have.been.called;

      // restore methods converted to spies and reset without_wemf flag for following tests
      window.az.globals.without_wemf = false;
      t.getTag = origGetTag;
      t.isLiveHost = origIsLiveHost;
      t._requestTag = orig_requestTag;
      t._log = orig_log;
