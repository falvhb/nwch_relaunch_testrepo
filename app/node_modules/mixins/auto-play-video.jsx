...
export default {
  ...
  setupOnScreenChangeHandlingOfPlayerWithoutAds() {
    ...
    if (typeof(window) !== 'undefined') {
      ...
      if (this.props.autoPlay && !isMobile()) {
        $(window).on('DOMContentLoaded load resize scroll', this.checkIfOnScreen);
      }
    }
  },
  ...
  checkIfOnScreen() {
    ...
    var isOnScreen = jQuery('#' + this.props.kalturaID).isOnScreen();
    if (isOnScreen !== this.state.onScreen) {
  ...
  setupPlayerWithAds() {
    ...
    var initPlayerWithAds = function(eventType) {
      var playerConfig = self.getPlayerConfiguration(self.props, true);
      self.setPlayerConfiguration(playerConfig);
      // listen for events to setup player with ads
      var j = self.playerEventsToWatch[eventType];
      var kdp = document.getElementById(self.props.kalturaID);
      var triggerPlayerReplacement = function() {

        if (self.state.inReplacementMode) {
          self.destroyVideo(self.props.kalturaID);
          self.embedVideo(self.state.playerConfig);
        }
      ...
      kdp.kBind(eventType, function() {
        if (eventType === 'doPause' || eventType === 'doSeek') {
    ...
    // setup players with ads
    _.forEach(this.playerEventsToWatch, initPlayerWithAds);
  },
